...
void engineRender(int pWidth, int pHeight) {;
  ...
  mat4Projection(&matrices.projection, 45.0, ratio, 1.0, 10000.0);
  ...
  // now render our FPS
    ...
    mat4Identity(&matrices.projection);
    mat4Ortho(&matrices.projection, -ratio * virtualScreenHeight, ratio * virtualScreenHeight, virtualScreenHeight, -virtualScreenHeight, 1.0f, -1.0f);
